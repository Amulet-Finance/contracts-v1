...
use cosmwasm_std::{
    coin, to_json_binary, Attribute, BankMsg, Coin, CosmosMsg, DecCoin, DistributionMsg, Env,
    QuerierWrapper, QueryRequest, Response, StakingMsg, Storage, Uint128, WasmMsg,
};
...
impl<'a> CoreStrategy for Strategy<'a> {
    ...
    fn deposit_value(&self, DepositAmount(amount): DepositAmount) -> DepositValue {
        // 1:1 (no exchange/redemption rate) because deposit asset == staking asset
        DepositValue(amount)
    }
...
struct TotalPendingRewards {
    validators_with_pending_rewards: Vec<String>,
    total_pending_rewards: u128,
}

fn query_total_pending_rewards(querier: QuerierWrapper, env: &Env) -> Result<TotalPendingRewards> {
    use cosmos_sdk_proto::cosmos::distribution::v1beta1::QueryDelegationTotalRewardsRequest;
    use prost::Message;

    let data = QueryDelegationTotalRewardsRequest {
    ...
    #[cw_serde]
    struct QueryDelegationTotalRewardsResponse {
        pub rewards: Vec<DelegationDelegatorReward>,
        pub total: DecCoin,
    }

    let res: QueryDelegationTotalRewardsResponse = querier.query(&QueryRequest::Stargate {
        path: "/cosmos.distribution.v1beta1.Query/DelegationTotalRewards".to_owned(),
        data,
    })?;

    let validators_with_pending_rewards = res
        .rewards
        .into_iter()
        .map(|d| d.validator_address)
        .collect();

    let total_pending_rewards = Uint128::try_from(res.total.amount.to_uint_floor())?.u128();

    Ok(TotalPendingRewards {
        validators_with_pending_rewards,
...
pub fn handle_collect_rewards(
    ...
    let TotalPendingRewards {
        validators_with_pending_rewards,
        total_pending_rewards,
    } = query_total_pending_rewards(querier, env)?;
    ...
    let withdraw_rewards_msgs = validators_with_pending_rewards
        .into_iter()
...
pub fn handle_process_redelegations(
    ...
    let validators_pending_redelegation = storage.validators_pending_redelegation();
    let active_validators = storage.validators();
    let bond_denom = storage.bond_denom();
        ...
        .take(limit)
    {
        ...
        let redelegate_msgs =
            distribute_delegations(&active_validators, delegation.can_redelegate.amount.u128())
                .map(|(dst_validator, amount)| StakingMsg::Redelegate {
...
pub fn handle_remove_validator(
    ...
    if !vault_validators.contains(&validator) {
        bail!("{validator} is not in the active validator set");
    }
    ...
    let redelegate_msgs =
        distribute_delegations(&vault_validators, delegation.can_redelegate.amount.u128()).map(
            |(dst_validator, amount)| StakingMsg::Redelegate {
